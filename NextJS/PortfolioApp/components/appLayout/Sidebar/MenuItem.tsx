  ...
  return (
    <CollapsiblePrimitive.Root className="transform transition duration-100 ease-in-out">
      <CollapsiblePrimitive.Trigger className="group w-full group-hover:text-white">
        <button
          className="group relative flex items-center justify-center gap-2.5 rounded-md   px-3 py-2    font-medium text-bodydark1 duration-300 ease-in-out  dark:hover:bg-meta-4">
          <div className="flex items-center justify-center">
            <div className="mr-2">{props.icon}</div>
            {
              <p
                className={`gorup-hover:block  text-lg group-hover:text-white ${!isSidebarOpen && 'hidden'}`}
              >
                {title}
              </p>
            }
